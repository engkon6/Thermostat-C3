...
void TH_Overlay() {
  ...
  display.setTextAlignment(TEXT_ALIGN_CENTER);
  if(tempValid)
    display.drawString(SCREEN_W / 2, 14, String(Temp, 1) + "°C");
  else
    display.drawString(SCREEN_W / 2, 14, "--.-°C");
  ...
  if (lastTempReceived > 0) {
    ...
    display.drawString(SCREEN_W / 2, 42, "Updated: " + String(buf));
  }
  ...
  if (setBlinkState)
    display.drawString(SCREEN_W / 2, 54, "Set: " + String(Temp2,1) + "°C");
  ...
  for (int i = 0; i < 4; i++) {
    ...
    if (reading == HIGH && btnState[i] == LOW) {
      ...
      if ((i == 0 || i == 1) && childUnlocked) {
        ...
        Temp2 = constrain(Temp2, -25.0, -15.0);
